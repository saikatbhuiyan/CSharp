...
var name = "Nisha"; //variable initialization at declaration
...
var @class = "First"; //we can use a reserved keyword if we precede it with @
var _number = 10;
var also_some_number = 5; //valid name, but not in line with C# convention
var alsoSomeNumber = 5; //C# uses lower camel case
//int 1number = 1; //will not compile, digit cannot be the first character
var name1 = "Anna"; //digit is fine if it is not the first character
...
//###################
var sum = 5 + 10;
var resultNoParenthesis = 5 + 2 * 3;
var resultParenthesis = 5 + 2 * 3;
...
//###################
var word1 = "text"; //explicitly typed variable
...
var isUserInputNotAbc2 = !(userInput == "ABC"); //will be the same as above
...
var isLargerThan5OrSmallerThan0 =
    number > 5 || number < 5; //the second conditin will not be evaluated due to short-circuiting
var isSmallerThanZeroAndEven =
    number < 0 && number % 2 == 0; //the second conditin will not be evaluated due to short-circuiting
...
if (userInput.Length == 0)
{
    ...
    var word = "ABC";
    var someNumber = 5;
    if (word.Length > 0) Console.WriteLine(someNumber); //someNumber is available here
}
else
{
    var
        someNumber =
            6; //named the same as the variable inside the "if". It is fine, because those variables live in different scopes
...
var parsedToInt = int.Parse(numberAsText); //would not work if input was, for example, "abc"
...
//For loop
//###################
for (var i = 0; i < 5; ++i) Console.WriteLine("Loop run " + i);
for (var i = 10; i >= 5; --i) Console.WriteLine("Loop run " + i);
Console.WriteLine("The loop is finished");
...
do
{
    ...
    if (input == "stop") break;
    var isParsableToInt = input.All(char.IsDigit);
    if (!isParsableToInt)
    {
        userNumber = 0;
        continue;
...
var sumOfNumbers = 0;
for (var i = 0; i < numbers.Length; i++) sumOfNumbers += numbers[i];


//###################
//Multi-dimensional arrays
//###################
var letters = new char[2, 3];
letters[0, 0] = 'A';
letters[0, 1] = 'B';
letters[0, 2] = 'C';
letters[1, 0] = 'D';
...
var containsOne = someWords.Contains("one");
someWords.Clear();
...
//###################
var variousNumbers = new[] { 10, -8, 2, 12, -17 };
...
//Out parameter
//###################
var isParsed = int.TryParse(
    userInput, out var userInputParsedToInt);
if (isParsed)
    Console.WriteLine(
